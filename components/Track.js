        ...
        <div className="flex items-center rounded-full border-2 border-[#262626] w-[85px] h-10 relative cursor-pointer group-hover:border-white/40">
          <AiFillHeart
            className={`text-xl ml-3 icon ${
              hasLiked ? 'text-[#cb5050]' : 'text-[#868686]'
            }`}
            onClick={() => setHasLiked(!hasLiked)}
          />
          {track.uri === playingTrack.uri && play ? (
            <div
              className="h-10 w-10 rounded-full border border-[#cb5050] flex items-center justify-center absolute -right-0.5 bg-[#cb5050] icon hover:scale-110"
              onClick={handlePlay}
            >
            ...
            <div
              className="h-10 w-10 rounded-full border border-white/60 flex items-center justify-center absolute -right-0.5 hover:bg-[#cb5050] hover:border-[#cb5050] icon hover:scale-110"
              onClick={handlePlay}
            >
